...
@Injectable()
export class LadActionLogYesterdayService extends BaseAspService implements LogService {
  // constructor(private readonly repository: LadActionLogRepository) {
  constructor() {
  super(LadActionLogYesterdayService.name);
  }
  ...
  private async performLadActionOperation(page: Page): Promise<RawLadData[]> {
    ...
    try {
      await writeToSpreadsheet({
        ...
        sheetName: "Lad_CV_Referrer_Yesterday_test",
