...
@Injectable()
export class LadStClickLogService extends BaseAspService implements LogService {
  // constructor(private readonly repository: LadClickLogRepository) {
  constructor() {
  super(LadStClickLogService.name);
  }
  ...
  const [download] = await Promise.all([
    ...
    try {
      await writeToSpreadsheet({
        ...
        spreadsheetId: process.env.SPREADSHEET_ID_LAD_MEN_CLICK || "",
        sheetName: "Lad_Click_Referrer_Today_ストラッシュ_test",
