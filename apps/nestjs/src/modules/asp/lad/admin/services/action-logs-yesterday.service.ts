...
export class LadAdminActionLogYesterdayService extends BaseAspService implements LogService {
  ...
  await page.waitForSelector('div[class^="csvInfoExport"]', { timeout: 15000 }).catch(() => {
    ...
    // // 🧠 対象期間に一致するリンクのCSVファイル名を取得
    // const matched = await page.evaluate((expectedDateStr) => {
    //   for (let i = 1; i <= 5; i++) {
    //     const el = document.querySelector(`div.csvInfoExport${i}`);
    //     if (el && el.textContent?.includes(expectedDateStr)) {
    //       console.log(`🧪 csvInfoExport${i} =`, el.textContent); 
    //       const link = el.querySelector('a[href^="javascript:void(0)"]');
    //       if (link) {
    //         const onclick = link.getAttribute('onclick');
    //         const match = onclick?.match(/"(.*?\.csv)"/);
    //         if (match && match[1]) {
    //           return {
    //             fileName: match[1],
    //             index: i,
    ...
    // if (!matched) {
    //   this.logger.warn(`対象期間「${expectedDateStr}」に一致するCSVリンクが見つかりませんでした。`);
    //   return [];
    // }
...
// // 🧠 対象期間に一致するリンクのCSVファイル名を取得（ログ付き）
// const matchedWithDebug = await page.evaluate((expectedDateStr) => {
//   const logs: string[] = [];
...
//   if (el) {
//     const text = el.textContent?.replace(/\s+/g, '') || '';
//     logs.push(`csvInfoExport${i}: ${text}`);

//     const rangeMatch = text.match(/\[対象期間:(\d{4}年\d{2}月\d{2}日)/);
//     const periodStartDate = rangeMatch?.[1];

//     console.log("🧪 periodStartDate:", periodStartDate);
//     console.log("🧪 expectedDateStr:", expectedDateStr);
//     console.log("🧪 match:", periodStartDate === expectedDateStr);

//     if (periodStartDate === expectedDateStr) {
//       const link = el.querySelector('a[href^="javascript:void(0)"]');
//       const onclick = link?.getAttribute('onclick');
//       const match = onclick?.match(/"(.*?\.csv)"/);
//       if (match && match[1]) {
...
//     }
//   } else {
//     logs.push(`csvInfoExport${i}: 要素なし`);
//   }
// }
...
// // 🧪 ログ出力
// matchedWithDebug.debugLogs?.forEach((log: string) => {
//   console.log("🧪 csvブロック確認:", log);
// });

// if (!matchedWithDebug.fileName) {
//   this.logger.warn(`対象期間「${expectedDateStr}」に一致するCSVリンクが見つかりませんでした。`);
//   return [];
// }
// const matchedFileName = matchedWithDebug.fileName;
// const targetSelector = `div.csvInfoExport${matchedWithDebug.index} a[href^="javascript:void(0)"]`;
//     // const matchedFileName = matched.fileName;
//     // const targetSelector = `div.csvInfoExport${matched.index} a[href^="javascript:void(0)"]`;
