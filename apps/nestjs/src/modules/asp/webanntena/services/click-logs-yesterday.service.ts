...
@Injectable()
export class WebanntenaClickLogYesterdayService extends BaseAspService implements LogService {
  constructor() {
    super(WebanntenaClickLogYesterdayService.name);
  }
  ...
  private async performWebanntenaClickOperation(page: Page): Promise<RawLadData[]> {
    ...
    try {
      await writeToSpreadsheet({
        ...
        sheetName: "TBC_click_yesterday_test",
