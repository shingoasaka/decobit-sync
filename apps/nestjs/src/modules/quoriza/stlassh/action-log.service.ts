...
export class StlasshActionLogService implements LogService {
  ...
  async fetchAndInsertLogs(): Promise<number> {
    ...
    try {
      ...
      await page.getByRole("textbox", { name: "Enter loginname" }).fill(process.env.STLASSH_USERNAME ?? "");
      await page.getByRole("textbox", { name: "Enter password" }).fill(process.env.STLASSH_PASSWORD ?? "");
